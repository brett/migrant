  ...
  if ! virsh dominfo "$VM_NAME" &>/dev/null; then
    ...
    case "$state" in
      ...
      crashed)
        ...
        printf '  %-*s%s\n' "$sw" 'note:' "run 'migrant.sh destroy' then 'migrant.sh up' to rebuild"
        ;;
      blocked)
