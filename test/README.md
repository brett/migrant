...
Run from any example VM directory (e.g.
`cd examples/arch && ../../test/test-hooks.sh`):
...
- **test-wireguard.sh** — WireGuard mode against a peer in a network namespace,
  ...
  must not order sshd behind `time-sync.target` — the example playbooks mask the
  NTP units, since kvm-clock already keeps guest time
