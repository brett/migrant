...
Run from `test/vm`, a bare VM directory kept for these scripts
(e.g. `cd test/vm && ../test-hooks.sh`).
...
- **test-wireguard.sh** — WireGuard mode against a peer in a network namespace,
  ...
  The VM must not order sshd behind `time-sync.target`: a tunnelled guest never
...
`playbook.yml`, so Ansible never runs. The scripts probe with bash's
`/dev/tcp` rather than `netcheck.py`, so nothing needs installing in the guest.
